...
- (void)setViewInterface {
    ...
    //    //获取当前时间 （时间格式支持自定义）
    NSString *currentDateStr = [NSString stringOfDate:nil];
    //拆分年月成数组
    ...
    //真实
    if ([UserModel ShareUserModel].isBuyService) {
...
- (void)buttonTapped:(UIButton *)sender {
    ...
    if (sender.tag == 10) {
        ...
        restr = [NSString stringWithFormat:@"%d-%02d",[selectedYear intValue],[selectecMonth intValue]];
        backBlock(restr);
        [self dismiss];
    }
}
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
    if (component == 0) {
        ...
        selectedYear = [selectedYear substringToIndex:selectedYear.length - 1];
